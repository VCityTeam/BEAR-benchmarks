...
DATASET_NAME="$DATASET_FULL_NAME" docker compose run --rm quads-query-cli-flat java -jar /opt/app/quads-query-cli.jar "$QUERY_PATH" > "$PROJECT_ROOT/experiment/results/${DATASET_FULL_NAME}-${QUERY_FILE%.rq}-result.txt"
